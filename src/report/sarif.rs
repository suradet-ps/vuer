...
fn rule_meta(id: &RuleId) -> RuleMeta {
  match id.as_str() {
    ...
    "vue/security/no-postmessage-wildcard" => RuleMeta {
      short: "Disallow `postMessage(..., '*')`",
      full: "A wildcard `targetOrigin` lets a malicious site that has navigated the target window intercept the message.",
      help: "Pin the receiver's exact origin (e.g. `https://app.example.com`) or use `/` for same-origin delivery.",
      severity: Severity::High,
      category: "security",
    },
    "vue/security/no-window-open-blank-noopener" => RuleMeta {
      short: "Require `noopener` on `window.open(..., '_blank', ...)`",
      full: "Without `noopener`, the opened tab can call `window.opener.location = ...` and phish the originating page (reverse tabnabbing).",
      help: "Add `noopener` (or `noreferrer`) to the `windowFeatures` string.",
      severity: Severity::High,
      category: "security",
    },
    "vue/security/no-fetch-without-timeout" => RuleMeta {
      short: "Require an `AbortSignal` on `fetch`",
      full: "A `fetch` without a signal can hang indefinitely and exhaust connection pools.",
      help: "Pass `signal: ctrl.signal` from an `AbortController` and call `ctrl.abort()` on timeout / unmount.",
      severity: Severity::High,
      category: "security",
    },
